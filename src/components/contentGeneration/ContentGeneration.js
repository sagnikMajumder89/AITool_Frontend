...
import { generateContent } from "../../apis/chatGPT";
...
const BlogPostAIAssistant = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      // Handle success, data is the mutation result
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      console.log(values);
      mutation.mutate({
        prompt: `Generate a blog post based: ${values.prompt}, ${values.tone}, ${values.category}`,
      });
    },
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        {mutation.isSuccess && (
          <StatusMessage
            type="success"
            message={"Successfully content generated"}
          />
        )}
