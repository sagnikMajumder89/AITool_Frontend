...
import { paymentVerify } from "../../apis/stripePayment";

const PaymentSuccess = () => {
  const [searchParams] = useSearchParams();

  // Static values for demonstration
  const paymentIntentID = searchParams.get("payment_intent"); // Example payment intent ID

  const { isLoading, isError, data, isPending, isSuccess } = useQuery({
    queryFn: () => paymentVerify(paymentIntentID),
  });
  console.log(data);
  return (
